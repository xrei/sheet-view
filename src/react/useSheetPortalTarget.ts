 ...
 *
 * A *closing* sheet still resolves to its own node: flipping the target mid-exit
 * reparents the content and restarts its animations. Read
 * `useSheetLayout().isClosing` to decide whether to render, or use `<SheetPortal>`.
 *
 * Client-only, like `open()`: on the server there is nothing to portal into.
 */
export function useSheetPortalTarget(
  options: SheetPortalTargetOptions = {}): HTMLElement {
  ...
  // The snapshot is the resolved node, not the entries array: the array's
  // identity changes on every emit while the layer node rarely does, and
  // useSyncExternalStore re-renders only when the snapshot changes (Object.is).
